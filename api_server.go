...
func (s *APIServer) handleKofiWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if config.API.KofiVerificationToken != "" {
		...
		var kofiData struct {
			...
			Type              string `json:"type"` // "Donation", "Subscription", "Shop Order"
			...
			TeirName string `json:"tier_name"`
		}
		...
		// Check if it is API key related purchase (single or subscription)
		if kofiData.TeirName != "Altbot Unlimited API Key" || (len(kofiData.ShopItems) > 0 && kofiData.ShopItems[0].DirectLinkCode != "a2d4aabd54") {
			s.jsonError(w, "Not an API key purchase", http.StatusBadRequest)
			return
		}
		...
		if existingKey != nil && existingKey.Active {
			...
			if err := ExtendAPIKey(existingKey.Key, duration); err != nil {
				log.Printf("Error extending API key for %s: %v", kofiData.Email, err)
				s.jsonError(w, "Failed to extend key", http.StatusInternalServerError)
				return
			}
			log.Printf("Extended API key for %s (Ko-fi %s)", kofiData.Email, kofiData.Type)
		} else {
			// Create new key
			note := fmt.Sprintf("Ko-fi %s from %s", kofiData.Type, kofiData.FromName)
			apiKey, err := GenerateAPIKey(kofiData.Email, duration, note)
			if err != nil {
				log.Printf("Error generating API key for %s: %v", kofiData.Email, err)
				s.jsonError(w, "Failed to generate key", http.StatusInternalServerError)
				return
			}
			log.Printf("Generated new API key for %s: %s... (Ko-fi %s)", kofiData.Email, apiKey.Key[:20], kofiData.Type)

			// TODO: Send email to user with their API key will use POSTMARK
	...
	// If no verification token configured, reject webhook
